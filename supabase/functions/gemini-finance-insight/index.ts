...
Rules:
1. Extract multiple transactions if present (separated by commas, "and", "juga", or from table format)
2. Categorize each transaction appropriately - use "Gift" category for money received from family/friends
3. Parse amounts in Indonesian format:
   - "ribu", "rb", "k" = multiply by 1000 (e.g., "10 ribu" = 10000)
   - "juta", "jt" = multiply by 1000000 (e.g., "2 juta" = 2000000)
   - IMPORTANT: Indonesian uses DOT as thousand separator! "100.000" = 100000 (one hundred thousand), NOT 100
   - "1.500.000" = 1500000, "50.000" = 50000, "150.000" = 150000
4. Default to expense type, UNLESS:
   - Words like "pemasukan", "income", "gaji", "salary", "terima", "dapat", "dari" indicate INCOME
   - Money received from people (e.g., "dari Ayah", "dari Mama", "dari Kakak") = INCOME type with "Gift" category
5. Return amount as INTEGER NUMBER (not string), without dots or formatting
...
Aturan:
1. Ekstrak multiple transaksi jika ada (dipisahkan koma, "dan", "juga", atau dari format tabel)
2. Kategorikan setiap transaksi dengan tepat - gunakan kategori "Gift" untuk uang yang diterima dari keluarga/teman
3. Parse jumlah dalam format Indonesia:
   - "ribu", "rb", "k" = kali 1000 (misal: "10 ribu" = 10000)
   - "juta", "jt" = kali 1000000 (misal: "2 juta" = 2000000)
   - PENTING: Format Indonesia menggunakan TITIK sebagai pemisah ribuan! "100.000" = 100000 (seratus ribu), BUKAN 100
   - "1.500.000" = 1500000, "50.000" = 50000, "150.000" = 150000
4. Default ke expense, KECUALI:
   - Kata seperti "pemasukan", "income", "gaji", "terima", "dapat", "dari" menunjukkan INCOME
   - Uang yang diterima dari orang (misal: "dari Ayah", "dari Mama", "dari Bude") = tipe INCOME dengan kategori "Gift"
5. Return amount sebagai INTEGER NUMBER (bukan string), tanpa titik atau formatting
6. Return response JSON dengan struktur tepat ini:
{
  "success": true,
...
Pastikan response adalah JSON yang valid dan amount adalah integer tanpa formatting.`;
}
