...
export const FinanceProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const addTransaction = async (transaction: Omit<Transaction, 'id' | 'userId'>) => {
    ...
    // CRITICAL: Fetch fresh wallet balances from DB to avoid stale closure issues
    // This is necessary when multiple transactions are added in quick succession (e.g., AI bulk add)
    const { data: freshWallets, error: walletFetchError } = await supabase
      .from('wallets')
      .select('id, balance')
      .eq('user_id', user.id);
    
    if (walletFetchError) {
      console.error('Error fetching fresh wallet balances:', walletFetchError);
      toast({ variant: 'destructive', title: 'Error', description: 'Failed to fetch wallet data.' });
      return;
    }

    // Create a map of fresh balances
    const freshBalanceMap = new Map<string, number>();
    freshWallets?.forEach(w => freshBalanceMap.set(w.id, w.balance));
    ...
    if (transaction.type === 'transfer') {
      ...
      // Use fresh balance from DB instead of stale local state
      const freshFromBalance = freshBalanceMap.get(fromWallet.id) ?? fromWallet.balance;
      const freshToBalance = freshBalanceMap.get(toWallet.id) ?? toWallet.balance;
      
      const fee = transaction.fee ?? 0;
      updatedFromWallet = { ...fromWallet, balance: freshFromBalance - (transaction.amount + fee) };
      updatedToWallet = { ...toWallet, balance: freshToBalance + transaction.amount };
      ...
      fromWallet = wallets.find(w => w.id === transaction.walletId);
       ...
       const newBalance = transaction.type === 'income'
           ? freshFromBalance + transaction.amount
           : freshFromBalance - transaction.amount;
    ...
    try {
      ...
      // CRITICAL FIX: Use functional update to avoid stale closure issues
      // When multiple transactions are added in quick succession (e.g., AI bulk add),
      // using 'wallets' from closure causes race conditions where only the last
      // transaction's balance is reflected. Using 'prev' ensures each update
