...
let globalToastFn: ((props: Toast) => { id: string; dismiss: () => void }) | null = null;
...
export const toast = (props: Toast) => {
  ...
  // Fallback: create toast directly in DOM
  const id = Math.random().toString();
  const container = ensureToastContainer();
  const toastElement = document.createElement("div");
  ...
  if (props.description) {
    const descEl = document.createElement("div");
    descEl.style.opacity = "0.9";
    descEl.textContent = props.description as string;
    toastElement.appendChild(descEl);
  }
...
export const registerToast = (fn: typeof globalToastFn) => {
  globalToastFn = fn;
}; 